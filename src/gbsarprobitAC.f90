subroutine gbsarprobitAC(ydata,wdata,xobs,nobs,nparw,nfun,nbasis,nint,fmodel,fpm,&
...
integer,intent(in) :: ydata(nobs),iflagpsi
...
integer :: iobs,iloop,ifun,xinxgrid(nobs,nfun),intsimpfacts(nint+1)
...
real(8) :: wtw(nparw,nparw),wtwi(nparw,nparw),xtx(nfun)
...
real(8) :: fxobs(nobs,nfun),fxgrid(nint+1,nfun),yest(nobs),yobs(nobs)

real(8) :: cdfnorm,rndnorm,ltnormrnd,rtnormrnd
...
do iobs=1,nobs
  if(ydata(iobs).eq.1) then
    yobs(iobs)=ltnormrnd(0.d0,1.d0,0.d0)
  else
    yobs(iobs)=rtnormrnd(0.d0,1.d0,0.d0)
  end if
end do
...
do imcmc=1,nmcmc
  ...
  if(imcmc.gt.nblow .and. mod(imcmc,nskip).eq.0) then
    ...
    yest=wb+sum(fxobs,2)
    do iobs=1,nobs
      muhatg(isave,iobs)=cdfnorm(yest(iobs),0.d0,1.d0,1,0)
    end do

    loglikeg(isave)=GetLogLik_Probit(ydata,yest,nobs)
...
do ifun=1,nfun
  ...
  if (fmodel(ifun).eq.1) then
    ...
    bn1=matmul(vn1,matmul(phixobst(2:(nbasis+1),:,ifun),rk))
...
do iobs=1,nobs
  if(ydata(iobs).eq.1) then
    yobs(iobs)=ltnormrnd(wb(iobs)+sum(fxobs(iobs,:)),1.d0,0.d0)
  else
    yobs(iobs)=rtnormrnd(wb(iobs)+sum(fxobs(iobs,:)),1.d0,0.d0)
  end if
end do
...
real(8) function GetLogLik_Probit(y,mu,nobs)
implicit none
...
GetLogLik_Probit=0.d0
do i=1,nobs
  if(y(i).eq.1) then
    GetLogLik_Probit=GetLogLik_Probit+cdfnorm(mu(i),0.d0,1.d0,1,1)
  else
    GetLogLik_Probit=GetLogLik_Probit+cdfnorm(mu(i),0.d0,1.d0,0,1)
  end if
end do

return
end function GetLogLik_Probit
...
do ifun=1,nfun
  ...
  if (fmodel(ifun).eq.5 .or. fmodel(ifun).eq.6 .or. fmodel(ifun).eq.7) then
    if (iflagpsi.eq.1) then
      lnpriorf=lnpriorf-((psi(ifun)-psi_m0)**2.d0)/(2.d0*psi_v0)-dlog(2.d0*PI*psi_v0)/2.d0-psi_lnp0
    end if
