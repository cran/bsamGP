subroutine bsaram(verbose,yobs,wdata,xobs,nobs,nparw,nfun,nbasis,nint,nExtremes,maxNext,fmodel,fpm,&
...
real(8) :: xgrid(nint+1,nfun),bhat(nparw),yhat(nobs),shat,xbar(nfun),xtx(nfun)
...
sigma2_rn=sigma2_r0+dble(nobs)+dble(nparw)
do ifun=1,nfun
...
do imcmc=1,nmcmc
  ...
  if(imcmc.gt.nblow .and. mod(imcmc,nskip).eq.0) then
    ...
    ehat=yobs-yest
    loglikeg(isave)=-sum(ehat**2.d0)/(2.d0*sigma2)-dble(nobs)*dlog(2.d0*PI*sigma2)/2.d0
...
!Internal arguments
real(8) :: gamrnd,rndunif,rtgamrnd,normrnd,ltnormrnd,rtnormrnd,tnormrnd,ltgamrnd
...
do ifun=1,nfun
  ...
  if (fmodel(ifun).eq.1) then
    ...
    bn1=matmul(vn1,matmul(phixobsfreet(:,:,ifree),rk))
