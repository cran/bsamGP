subroutine bsaramdpscale(yobs,wdata,xobs,nobs,nparw,nfun,nbasis,nint,fmodel,fpm,&
                         ...
                         egrid,ngrid,iflagprior,iflagpsi,&
                         maxmodmet,nblow0,nblow,smcmc,nskip,ndisp,&
                         zetag,tau2g,gammag,thetag,betag,alphag,sigma2g,psig,omegag,&
...
real(8),intent(out) :: invlikeg(smcmc,nobs),pmetg(nfun),tmassg(smcmc),edensg(smcmc,ngrid)
...
integer :: nclass,ss(nobs)
integer,allocatable :: nh(:)
real(8) :: invlike(nobs),sigma2(nobs),sigmai(nobs,nobs),tmass,edens(ngrid)
real(8), allocatable :: sigma2h(:)

real(8) :: cdfnorm,rndnorm,gamrnd
...
ss=(/ (iobs,iobs=1,nobs) /)
tmass=1.d0
sigma2=(/ (1.d0/gamrnd(sigma2_r0/2.d0,2.d0/sigma2_s0),iobs=1,nobs) /)
...
!Internal arguments
real(8) :: gamrnd,rndunif,normrnd,ltnormrnd,rtnormrnd,tnormrnd,ltgamrnd
...
resid=yobs-wb-sum(fxobs,2)
call dp_update_configs_nogaps(resid,tmass,sigma2_r0,sigma2_s0,nobs,nclass,ss,sigma2)

allocate(sigma2h(nclass),nh(nclass))
call dp_update_classscale(resid,ss,sigma2_r0,sigma2_s0,nobs,nclass,sigma2h,nh,sigma2)
call diagvec(1.d0/sigma2,nobs,sigmai)

call dp_update_totalmass(tmass_a,tmass_b,nclass,nobs,tmass)

if(imcmc.gt.nblow .and. mod(imcmc,nskip).eq.0) then
  call dp_compute_invlike(resid,nh,sigma2h,tmass,sigma2_r0,sigma2_s0,nclass,nobs,invlike)
  call dp_update_predictive_dens(egrid,nh,sigma2h,tmass,sigma2_r0,sigma2_s0,nclass,nobs,ngrid,edens)
end if
deallocate(sigma2h,nh)

return
end subroutine GetMCMC


subroutine dp_update_configs_nogaps(resid,tmass,asig,bsig,nobs,nclass,classind,sigma2)
implicit none

! Input arguments
integer,intent(in) :: nobs
real(8), intent(in) :: resid(nobs),asig,bsig,tmass
...
do iobs=1,nobs
  ...
  if (uns.eq.1) then
    ...
    do iJ=1,(J_i+1)
      loglik(iJ)=dnrm(resid(iobs),0.d0,dsqrt(sigStar(iJ)),1)
    end do
...
subroutine dp_update_classscale(obs,ss,nu0,s0,nobs,nclass,class_v,nclassh,obs_v)
implicit none

!input arguments
integer,intent(in) :: nobs,nclass,ss(nobs)
real(8), intent(in) :: obs(nobs),nu0,s0

!output arguments
real(8), intent(out) :: class_v(nclass),obs_v(nobs)
integer,intent(out) :: nclassh(nclass)

!internal arguments
integer :: iobs,h
real(8) :: sobshsq,var_rh,var_sh,gamrnd
...
subroutine dp_compute_invlike(obs,nclassh,sigma2h,tmass,nu0,s0,nclass,nobs,invlike)
implicit none

!input arguments
integer,intent(in) :: nclass,nobs,nclassh(nclass)
real(8), intent(in) :: obs(nobs),sigma2h(nclass),tmass,nu0,s0
...
!internal arguments
integer :: iobs,iclass
real(8) :: sigma2_new,ppf(nclass+1),like(nobs),gamrnd,dnrm
...
do iobs=1,nobs
  do iclass=1,nclass
    like(iobs)=like(iobs)+ppf(iclass)*dnrm(obs(iobs),0.d0,dsqrt(sigma2h(iclass)),0)
  end do
  sigma2_new=1.d0/gamrnd(nu0/2.d0,2.d0/s0)
  like(iobs)=like(iobs)+ppf(nclass+1)*dnrm(obs(iobs),0.d0,dsqrt(sigma2_new),0)
end do
...
subroutine dp_update_predictive_dens(egrid,nclassh,sigma2h,tmass,nu0,s0,nclass,nobs,ngrid,edens)
implicit none

!input arguments
integer,intent(in) :: nclass,ngrid,nobs,nclassh(nclass)
real(8), intent(in) :: egrid(ngrid),sigma2h(nclass),tmass,nu0,s0
...
real(8) :: sigma2_new,ppf(nclass+1),gamrnd,dnrm
...
do igrid=1,ngrid
  do iclass=1,nclass
    edens(igrid)=edens(igrid)+ppf(iclass)*dnrm(egrid(igrid),0.d0,dsqrt(sigma2h(iclass)),0)
  end do
  sigma2_new=1.d0/gamrnd(nu0/2.d0,2.d0/s0)
  edens(igrid)=edens(igrid)+ppf(nclass+1)*dnrm(egrid(igrid),0.d0,dsqrt(sigma2_new),0)
end do

return
end subroutine dp_update_predictive_dens

end subroutine bsaramdpscale
