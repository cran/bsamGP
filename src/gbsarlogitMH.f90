subroutine gbsarlogitMH(verbose,yint,wdata,xobs,nobs,nparw,nfun,nbasis,nint,fmodel,fpm,&
...
do imcmc=1,nmcmc
  ...
  if(imcmc.gt.nblow .and. mod(imcmc,nskip).eq.0) then
    ...
    loglikeg(isave)=GetLogLik_Logit(yobs,yest,nobs)
...
beta_old=beta
wb_old=wb
loglik_old=GetLogLik_Logit(yobs,wb_old+sum(fxobs,2),nobs)
...
loglik_new=GetLogLik_Logit(yobs,wb_new+sum(fxobs,2),nobs)
...
do ifun=1,nfun
  ...
  loglik_new=GetLogLik_Logit(yobs,rk+fxobs_new,nobs)
  loglik_old=GetLogLik_Logit(yobs,rk+fxobs(:,ifun),nobs)
...
do ifun=1,nfun
  ...
  if(fmodel(ifun).gt.2 .and. fmodel(ifun).lt.7) then
    ...
    loglik_old=GetLogLik_Logit(yobs,rk+alpha_old*(xobs(:,ifun)-xmid(ifun)),nobs)
    sold=(alpha_old-alpha_m0)**2.d0

    alpha_new=normrnd(alpha_old,dexp(a_lsd(ifun)))
    loglik_new=GetLogLik_Logit(yobs,rk+alpha_new*(xobs(:,ifun)-xmid(ifun)),nobs)
...
real(8),intent(in) :: y(nobs),mu(nobs)

GetLogLik_Logit=sum(y*mu-dlog(1.d0+dexp(mu)))
