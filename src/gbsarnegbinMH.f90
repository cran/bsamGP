subroutine gbsarnegbinMH(verbose,yint,wdata,xobs,nobs,nparw,nfun,nbasis,nint,fmodel,fpm,&
...
integer :: iloop,ifun,intsimpfacts(nint+1),xinxgrid(nobs,nfun)
...
integer :: nbatch,ibatch,icount_alpha,a_nacc(nfun)
integer :: kappa_nacc
...
!Internal arguments
real(8) :: dgamm,gamrnd,rndunif,normrnd,ltnormrnd,tnormrnd,ltgamrnd
...
beta_old=beta
wb_old=wb
loglik_old=GetLogLik_NegBin(yobs,dexp(wb_old+sum(fxobs,2)),kappa,nobs)
...
loglik_new=GetLogLik_NegBin(yobs,dexp(wb_new+sum(fxobs,2)),kappa,nobs)
...
do ifun=1,nfun
  ...
  loglik_new=GetLogLik_NegBin(yobs,dexp(rk+fxobs_new),kappa,nobs)
  loglik_old=GetLogLik_NegBin(yobs,dexp(rk+fxobs(:,ifun)),kappa,nobs)
...
do ifun=1,nfun
  ...
  if(fmodel(ifun).gt.2 .and. fmodel(ifun).lt.7) then
    ...
    loglik_old=GetLogLik_NegBin(yobs,dexp(rk+alpha_old*(xobs(:,ifun)-xmid(ifun))),kappa,nobs)
    sold=(alpha_old-alpha_m0)**2.d0

    alpha_new=normrnd(alpha_old,dexp(a_lsd(ifun)))
    loglik_new=GetLogLik_NegBin(yobs,dexp(rk+alpha_new*(xobs(:,ifun)-xmid(ifun))),kappa,nobs)
...
kappa_old=kappa
loglik_old=GetLogLik_NegBin(yobs,dexp(wb+sum(fxobs,2)),kappa_old,nobs)

kappa_new=normrnd(kappa_old,dexp(kappa_lsd))
loglik_new=GetLogLik_NegBin(yobs,dexp(wb+sum(fxobs,2)),kappa_new,nobs)

kappa_testp=loglik_new+dgamm(kappa_new,kappa_r0,1.d0/kappa_s0,1)- &
loglik_old-dgamm(kappa_old,kappa_r0,1.d0/kappa_s0,1)
